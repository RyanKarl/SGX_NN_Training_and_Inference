...
int main(int argc, char ** argv){
  ...
  if(!n){
    cout << "ERROR: argue a number of iterations with -n\n";
    return 0;
  }
  if(!s){
    cout << "ERROR: argue a number of bytes with -s\n";
    return 0;
  }
  if(s % sizeof(float)){
    cout << "ERROR: number of bytes argued doesn't cleanly divide into floats\n";
    return 0;
  }

  reserve_vec(n/sizeof(float));
...
#ifdef NENCLAVE
  int enclave_result = enclave_main(n, s);
