...
sgx_status_t SGX_CDECL session_request_ocall(uint32_t* retval, sgx_enclave_id_t src_enclave_id, sgx_enclave_id_t dest_enclave_id, sgx_dh_msg1_t* dh_msg1, uint32_t* session_id)
{
	...
	if (dh_msg1 != NULL) {
		...
		__tmp_dh_msg1 = __tmp;
		if (memcpy_s(__tmp_dh_msg1, ocalloc_size, dh_msg1, _len_dh_msg1)) {
			sgx_ocfree();
			return SGX_ERROR_UNEXPECTED;
		}
	...
	if (session_id != NULL) {
		...
		if (memcpy_s(__tmp_session_id, ocalloc_size, session_id, _len_session_id)) {
			sgx_ocfree();
			return SGX_ERROR_UNEXPECTED;
		}
...
sgx_status_t SGX_CDECL exchange_report_ocall(uint32_t* retval, sgx_enclave_id_t src_enclave_id, sgx_enclave_id_t dest_enclave_id, sgx_dh_msg2_t* dh_msg2, sgx_dh_msg3_t* dh_msg3, uint32_t session_id)
{
	...
	if (dh_msg2 != NULL) {
		...
		__tmp_dh_msg2 = __tmp;
		if (memcpy_s(__tmp_dh_msg2, ocalloc_size, dh_msg2, _len_dh_msg2)) {
			sgx_ocfree();
			return SGX_ERROR_UNEXPECTED;
		}
	...
	if (dh_msg3 != NULL) {
		...
		__tmp_dh_msg3 = __tmp;
		if (memcpy_s(__tmp_dh_msg3, ocalloc_size, dh_msg3, _len_dh_msg3)) {
			sgx_ocfree();
			return SGX_ERROR_UNEXPECTED;
		}
